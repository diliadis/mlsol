...
# a zero value indicates a safe neighborhood and a value of one indicates a hostile neighborhood
def get_C(y, indices):
    C = np.zeros(y.shape)
...
def get_weight_per_example(y, C, min_class_per_label):
...
def get_type_matrix(y, C, min_class_per_label, neighbour_indices):
...
def mlsol_main(X, y, perc_gen_instances=0.3, k=5):
    ...
    C = get_C(y, indices)
    w = get_weight_per_example(y, C, min_class_per_label)
    T = get_type_matrix(y, C, min_class_per_label, indices)
    counter = 0
    init_size = gen_num
    while gen_num > 0:
        ...
        X_synthesized[gen_num-1], y_synthesized[gen_num-1] = generate_instance(X[seed_index, :], y[seed_index, :], T[seed_index, :], X[reference_index, :], y[reference_index, :], T[reference_index, :])
...
def main():
    ...
    X_aug, y_aug = mlsol_main(X, Y, perc_gen_instances=0.3, k=5)
